...
{
    /// <summary>
    /// do_sljsjj_Sljsjj_loadSljsjjList 的摘要说明
    /// </summary>
    public class do_sljsjj_Sljsjj_loadSljsjjList : IHttpHandler,IReadOnlySessionState
    {
                ...
                if (userysbqclist.Count > 0)
                {
                    foreach (GTXGXUserYSBQC item in userysbqclist)
                    {
                        if (item.reportid == "bbtb_sljsjj"
                            && (DateTime.Compare(Convert.ToDateTime(item.SKSSQQ), Convert.ToDateTime(sssq_q)) >= 0)
                            && (DateTime.Compare(Convert.ToDateTime(sssq_z), Convert.ToDateTime(item.SKSSQZ)) >= 0)
                            && item.SBZT == "已申报")
                        {
