...
function _setData2Row(table, datas, index) {
    ...
    for (var i = 0; i < rows.length; i++) {
        ...
        for (var j = 0; j < r.cells.length; j++) {
            ...
            if ((cell.getAttribute("writable") || es == '6' || es == '8' || es == '11' || es == '12')) { //&& ( cell.style.display != 'none' && cell.parentElement.style.display != 'none' ) ) {
...
function _calcTbl(table, changedCell) {
    ...
    if (changedCell == null) {  //导入Excel结束时调用
        var topTbl = document.getElementById(table.id + "_$_top");
        if (topTbl == null) changedCell = document.getElementById(table.currCell.parentElement.getAttribute("pid")).rows[0].cells[0];
