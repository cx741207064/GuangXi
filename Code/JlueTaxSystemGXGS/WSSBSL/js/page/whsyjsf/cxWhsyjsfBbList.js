...
function loadWhsyjsfList() {
    ...
    if (cxsjQ == "" || cxsjZ == "") {
        ...
        $.postJSON("/WSSBSL/do_whsyjsf_Whsyjsf_loadWhsyjsfList.ashx", {
            "cxsjQ": cxsjQ,
            "cxsjZ": cxsjZ
        },
...
function showDqysbList(list) {
    ...
    if (list && list.length > 0) {
        for (var i = 0; i < list.length; i++) {
            ...
            lis.push('<td height="15" align="center" ><a href="javascript:;" onclick="chooseTr(this);showReport(\'' + list[i].tbqk + '\',\'' + list[i].dataFromBz + '\',\'' + list[i].pzxh + '\',\'' + list[i].sbuuid + '\')">' + list[i].bbzlmc + '</a></td>');
...
//显示报表
function showReport(tbqk, dataFromBz, pzxh, sbuuid) {
    closeAllTab();
    ...
    if (typeof (nsrsbh) == "undefined") {
        ...
        if (tbqk.substring(0, 1) == '1') {
            addTab("主表", "/WSSBSL/reportJsp/showCxReport_whsyjsfsbb.aspx?raq=/whsyjsf/whsyjsfsbb.raq&nsrsbh=" + nsrsbh + "&isTbBz=" + isTbBz + "&dataFromBz=" + dataFromBz + "&pzxh=" + pzxh + "&sbuuid=" + sbuuid, false);
        }
        if (tbqk.substring(1, 2) == '1') {
            addTab("附表一", "/WSSBSL/reportJsp/showCxReport_ysfwkcxmqd.aspx?raq=/whsyjsf/ysfwkcxmqd.raq&nsrsbh=" + nsrsbh + "&isTbBz=" + isTbBz + "&dataFromBz=" + dataFromBz + "&pzxh=" + pzxh + "&sbuuid=" + sbuuid, false);
        }
    }
    var options = {
        onSelect: function (title) {
