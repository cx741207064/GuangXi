...
Document.implement({
    ...
    newElement: function (tag, props) {
        if (Browser.Engine.trident && Browser.Engines.trident() < 6 && props) {
...
Window.implement({
    ...
    $$: function (selector) {
        ...
        var elements = [], doc = this.document;;
        var args = Array.flatten(arguments);
        for (var i = 0, l = args.length; i < l; i++) {
            var item = args[i], tType = $type(item);
            if (tType == "element") {
                elements.push(item);
            } else if (tType == "string") {
                elements.extend(doc.getElements(item, true));
            }
        }
...
(function () {
    ...
    Element.implement({
        ...
        getComputedStyle: function (property) {
            ...
            //var computed = this.getDocument().defaultView.getComputedStyle(this, null);  //确认的bug
            var computed = (this.getDocument().defaultView) ? this.getDocument().defaultView.getComputedStyle(this, null) : null;
        ...
        empty: function () {
            ...
            //                  try{
            //					  var teD=this.contentWindow.document;
            //					  if(teD){teD.write('');
            //                      teD.close();}
            //				  }catch(e){}
            //			}
...
(function () {
    Element.implement({
        /*expose measure isVisible 方法copy于mootools1.5 ，获取display为none状态DOM元素的宽、高*/
        expose: function () {
...
Fx.Scroll = new Class({
    ...
    calculateScroll: function (x, y) {
        ...
        for (var z in values) {
            ...
            if ($type(values[z]) != 'number') values[z] = scrollSize[z] - size[z];
            values[z] += offset[z];
        }
    ...
    toElement: function (el, axes) {
        ...
        var position = $(el).getPosition(this.element);
        var isBody = (/^(?:body|html)$/i).test(this.element.tagName);
        var scroll = isBody ? { x: 0, y: 0 } : this.element.getScroll();
        var t = new Hash(position).each(function (value, axis) {
            position[axis] = axes.contains(axis) ? value + scroll[axis] : false;
        });
        var to = this.calculateScroll(position.x, position.y);
        return this.start.apply(this, to);
        //return this.start(position.x, position.y);
    }
});
