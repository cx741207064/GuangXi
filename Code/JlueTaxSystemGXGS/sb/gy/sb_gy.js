 ...
 */
function qzDate(skssqq, skssqz, today, nsqxdm) {
    var succMsg = qzDateCheck(skssqq, skssqz, today, nsqxdm);
    if (succMsg == "ok") {
        return true;
    } else {
        swordAlert(succMsg);
...
function qzDateCheck(skssqq, skssqz, today, nsqxdm) {
 ...
 * @return
 */
function changeSblx(option, selItem, obj) {
...
function getMrjmseBySsjmxz(jmzlxDm, jsyj, ynse, fdsl, jmfd, jmed, jmsl) {
    ...
    if (ynse != jsyj.multiple(fdsl)) {//非按照税率计税的情况,此时对于税率减免和幅度减免不做减免税额计算，需要前台手工录入减免税额
        flag = true;
    }
    ...
    for (var i = 0; i <= num; i++) {
        ...
        if (!$chk(dis) || dis != "none") {
            ...
            if (0 == i) {
                ...
                var btns = getClassName(document.getElementById(tab + i + "iframe").contentWindow.document.body, 'submit_btn');
                var btnDis = [];
                for (var btni = 0; btni < btns.length; btni++) {
                    btnDis.push(btns[btni].style.display);
                    btns[btni].style.display = "none";
                }
                inhtml = document.getElementById(tab + i + "iframe").contentWindow.document.body.innerHTML;
                for (var btni = 0; btni < btns.length; btni++) {
                    btns[btni].style.display = btnDis[btni];
                }
                //去掉注释
                inhtml = inhtml.replace(/<!--[\w\W\r\n]*?-->/gmi, '');
            }

            //    			fso = new ActiveXObject("Scripting.FileSystemObject" );
            //    			var f=fso.createtextfile("d:\\testfile1.txt" ,2,true);
            //    			f.writeLine(inhtml);
            //    			f.close();
            ...
            while (inhtml.indexOf("<") >= 0) {
                ...
                if (oldc.indexOf("tb_mini_box") >= 0) {
                    oldc = oldc.replace("tb_mini_box", "tb_mini_box display= none");
                }
                if (oldc.indexOf("</") >= 0) {
                    ...
                    if (oldc.indexOf(" ") >= 0) {
                        ...
                        for (var num = 0; num < li.length; num++) {
                            var cc = li[num];
                            if (cc.indexOf("DISPLAY") >= 0 || cc.indexOf("display") >= 0) {
                                disp = li[num + 1];
                                disp = disp.replace(">", "");
                                disp = disp.replace(/\"/g, "");
                                existdis = "true";
                            }
                            if (cc.indexOf("value") >= 0 || cc.indexOf("VALUE") >= 0) {
                                if (cc.indexOf("value") >= 0) {
                                    va = cc.substring("value" + 1, cc.length);
                                } else {
                                    va = cc.substring(cc.indexOf("VALUE") + 1, cc.length);
                                }
                                va = va.replace(">", "");
                                va = va.replace(/\"/g, "");
                                va = va.replace("value=", "");
                            ...
                            //rowspan
                            if (cc.indexOf("rowSpan") >= 0) {
                                rowsp = cc.substring(cc.indexOf("rowSpan") + 8, cc.length);
                                rowsp = rowsp.replace(">", "");
                                rowsp = rowsp.replace(/\"/g, "");
                                rowspanex = "true";
                            }
                            //colspan
                            if (cc.indexOf("colSpan") >= 0) {
                                colsp = cc.substring(cc.indexOf("colSpan") + 8, cc.length);
                                colsp = colsp.replace(">", "");
                                colsp = colsp.replace(/\"/g, "");
                                colspanex = "true";
                            }
                        }
                        if ("true" == existdis && ("none>" == disp || "NONE>" == disp || "none" == disp || "NONE" == disp)) {
                            var bigintag = "<" + oldc.substring(1, oldc.indexOf(" "));
                            var endtag = "</" + oldc.substring(1, oldc.indexOf(" ")) + ">";
                        ...
                        } else if ("true" == existva && !(oldc.indexOf("LABEL") >= 0 || oldc.indexOf("lable") >= 0)) {
                            var sp = " colSpan='" + colsp + "' rowSpan='" + rowsp + "'";
                            var str = "∪" + oldc.substring(1, oldc.indexOf(" ")) + "∩" + va;
                            var rpstr = "∪" + oldc.substring(1, oldc.indexOf(" ")) + sp + "∩" + va;
                            if (oldc.indexOf("sword=\"SwordGrid\"") >= 0) {
                                ...
                                var cpht2 = cpht.replace(new RegExp(/(<)/g), '∪');
                                cpht2 = cpht2.replace(new RegExp(/(>)/g), '∩');
                                inhtml = inhtml.replace(cpht, cpht2);
                            } else if (oldc.indexOf("<TD") >= 0 || oldc.indexOf("<td") >= 0 || oldc.indexOf("<TH") >= 0 || oldc.indexOf("<th") >= 0 || oldc.indexOf("<TR") >= 0 || oldc.indexOf("<tr") >= 0) {
                                inhtml = inhtml.replace(oldc, rpstr);
                            } else {
                                inhtml = inhtml.replace(oldc, str);
                            }
                        } else {
                            var sp = " colSpan='" + colsp + "' rowSpan='" + rowsp + "'";
                            var str = "∪" + oldc.substring(1, oldc.indexOf(" ")) + "∩";
                            var rpstr = "∪" + oldc.substring(1, oldc.indexOf(" ")) + sp + "∩";
                            if (oldc.indexOf("sword=\"SwordGrid\"") >= 0) {
                                var bigintag = "<" + oldc.substring(1, oldc.indexOf(" "));
                                var endtag = "</" + oldc.substring(1, oldc.indexOf(" ")) + ">";
                                var endnum = 0;
            ...
            inhtml = inhtml.replace(new RegExp(/(<IMG>)/g), '');
            inhtml = inhtml.replace(new RegExp(/(正在加载......)/g), '');
            inhtml = inhtml.replace(new RegExp(/(\*)/g), '');
            inhtml = inhtml.replace(new RegExp(/(<SPAN><\/SPAN>)/g), '');
            inhtml = inhtml.replace(new RegExp(/(数字,且不能为空)/g), '');
            if (inhtml.indexOf("<CENTER>") >= 0) {
                var center = inhtml.substring(inhtml.indexOf("<CENTER>"), inhtml.indexOf("</CENTER>") + 9);
                inhtml = inhtml.replace(center, "");
            }
            if (inhtml.indexOf("<STYLE>") >= 0) {
                var center = inhtml.substring(inhtml.indexOf("<STYLE>"), inhtml.indexOf("</STYLE>") + 8);
                inhtml = inhtml.replace(center, "");
            }
            inhtml = inhtml.replace(new RegExp(/(null)/g), "");
            //
            //    			css = css + "table{border-collapse:collapse;width:100%;}table,td,th{border:1px solid black;}" +
            //    					".sGrid_data_row_item_div {display:block;float: leftborder-right: 10px solid;margin-left:-1px;overflow:hidden;white-space:nowrap;}"
            //    			+ "</style>";

            var strStyleCSS = "<link href='../../sb/sb000/zlbs.css' type='text/css' rel='stylesheet'>";
            //    			fso = new ActiveXObject("Scripting.FileSystemObject" );
            //    			var f=fso.createtextfile("d:\\testfile.txt" ,2,true);
            //    			f.writeLine(inhtml);
            //    			f.close();
...
function zlbstofb(name, zlbsxlDm, nsrxxMap) {
    ...
    if (!$chk(isopen)) {
        ...
        iframe.src = AddBizCode2URL("/sword.ashx?ctrl=SBGyCtrl_toCwbbSbfb&id=" + name + "&zlbsxlDm=" + zlbsxlDm);;
...
function setCfcjData(resData) {
    ...
    //	zlbsfbsaveData.set( "费用按照性质分类的补充资料" ,fyazxzfldbcclGrid);
    ...
    //zlbsfbsaveData.set( "存货的披露（适用执行企业会计制度的企业）" ,chdplGrid);
    ...
    //	zlbsfbsaveData.set( "固定资产的披露（适用执行企业会计制度的企业）" ,"a");
    var yfzcfzzhzbGrid = pc.getResData("yfzcfzzhzbGrid", resData);
    ...
    var yfzcfzzhzbForm = pc.getResData("yfzcfzzhzbForm", resData);
    if (yfzcfzzhzbForm) {
