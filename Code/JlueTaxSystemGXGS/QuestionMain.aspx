<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="QuestionMain.aspx.cs" Inherits="JlueTaxSystemGXGS.QuestionMain" %>
...
<meta http-equiv="Content-Type" content="text/html; charset=utf-8" />
    ...
    <script>
        ...
        $(document).ready(function () {
            $.ajax({
                url: "/VideoManage/GetVideo.ashx?CourseId=" + '<%=System.Web.Configuration.WebConfigurationManager.AppSettings["CourseId"]%>',
                type: "GET",
                dataType: "text",
                async: false,
        ...
        $(document).ready(function () {
            $.ajax({
                ...
                dataType: "jsonp",
                data: {
                    CourseId: '<%=System.Web.Configuration.WebConfigurationManager.AppSettings["CourseId"]%>',
                    classid: ClassId,
                    userid: userId,
                    sortid: sortid
                },
                ...
                success: function (result) {
                    if (typeof (result.Data) == "string") {
                        ...
                        $.each(json, function (i, v) {
                            ...
                            var Gtxpath = "WSSBSL/show_index_index.aspx?userid=" + userId + "&username=" + username + "&classid=" + ClassId + "&courseid=" + courseid + "&sortid=" + sortid + "&questionId=" + v.QuestionId + "&userquestionId=" + v.Id + "&CompanyId=" + v.CompanyId + "&rand=" + new Date().getTime();
                            var Dtxpath = '<%=System.Web.Configuration.WebConfigurationManager.AppSettings["Dtxpath"]%>' + "/index.php?sessionId=" + v.NSRSBH + "&questionid=" + v.QuestionId + "&happendate=" + v.HappenDate + "&userid=" + userId + "&classid=" + ClassId + "&CompanyId=" + v.CompanyId;
                            ...
                            str += '</div>';
                            if (v.VideoID) {
                                str += '<div id="oper' + v.QuestionId + '" style="display: none" class="oper_area"><a class="radius box-shadow ksmc-sj1-tanchuti"  onclick="checkpath(\'' + downpath + '\',\'' + v.FilePath + '\')" target="_blank" style="display:none">下载材料</a><a class="radius box-shadow ksmc-sj1-tanchuti" href="' + Gtxpath + '" target="_blank" onclick="closeareaGTX(' + v.QuestionId + ');">国税申报</a><a class="radius box-shadow ksmc-sj1-tanchuti" target="_blank" href="' + Dtxpath + '"  onclick="closeareaDTX();">地税申报</a><a target="_blank" class="radius box-shadow ksmc-sj1-tanchuti" onclick="checkvideo(\'' + Gtxvideopath + '\',\'' + v.VideoID + '\',\'国\')" >国税视频</a><a class="radius box-shadow ksmc-sj1-tanchuti" target="_blank" onclick="checkvideo(\'' + Dtxvideopath + '\',\'' + v.VideoID + '\',\'地\')">地税视频</a><span id="TX' + v.QuestionId + '" class="ksmc-cssecond"></span></div>';
                            }
                            else {
                                str += '<div id="oper' + v.QuestionId + '" style="display: none" class="oper_area"><a class="radius box-shadow ksmc-sj1-tanchuti"  onclick="checkpath(\'' + downpath + '\',\'' + v.FilePath + '\')" target="_blank" style="display:none">下载材料</a><a class="radius box-shadow ksmc-sj1-tanchuti" href="' + Gtxpath + '" target="_blank" onclick="closeareaGTX(' + v.QuestionId + ');">国税申报</a><a class="radius box-shadow ksmc-sj1-tanchuti" target="_blank" href="' + Dtxpath + '"  onclick="closeareaDTX();">地税申报</a><span id="TX' + v.QuestionId + '" class="ksmc-cssecond"></span></div>';
                            }
                            str += '</li>'
                            $("#ulTax").append(str);
                        });
        ...
        function togglearea(id, obj) {
            ...
            if (show) {
                $('#' + id).slideDown('slow');
                $.ajax({
                    url: '<%=System.Web.Configuration.WebConfigurationManager.AppSettings["tikupath"]%>' + GetTaxStatus,
                    type: "post",
                    dataType: "jsonp",
                    data:
                        {
                            classid: ClassId,
                            Userid: userId,
                            QuestionId: id.replace("oper", "")
                        },
                    async: false,
                    success: function (result) {
                        $('#TX' + id.replace("oper", "")).text(result.Data);
                    }
                });
            }
            else {
