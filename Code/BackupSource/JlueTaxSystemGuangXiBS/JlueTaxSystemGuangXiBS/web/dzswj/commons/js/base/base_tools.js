...
var baseTools = (function() {
	...
	return {
		...
		xhrAjaxJsonP : function(xhrArgs) {
            ...
            if (!xhrArgs.params){
                xhrArgs.params = {CUR_USERID : baseTools.getUserIdSeq()||"-1"};
            } else if (xhrArgs.params && !xhrArgs.params.CUR_USERID) {
                xhrArgs.params.CUR_USERID = baseTools.getUserIdSeq()||'-1';
            }
...
//				urlParam += "&SJQXFA=" + ck["SJQXFA"];
		...
		xhrAjax : function(xhrArgs) {
            ...
            if (!(xhrArgs.url.indexOf("http://") == 0 || xhrArgs.url
                ...
                // 客户端请求
                    xhrArgs.url = jcptTools.cnf.clientURL + xhrArgs.url;
                else
                // 支持二级目录部署
                    xhrArgs.url = jcptTools.cnf.contextPath + xhrArgs.url;
            }
			...
			$.ajax({
				...
				// 当请求失败时调用的函数
				error : function(XMLHttpRequest, textStatus, errorThrown) {
                    if (xhrArgs.isShowErrorMsg == undefined || xhrArgs.isShowErrorMsg == 'undefined' || xhrArgs.isShowErrorMsg == true){
						// 调用本次AJAX请求时传递的options参数
                        if(textStatus == 'timeout'){
                            alert('连接服务器超时，请稍后再试！' );
                        }else{
...
baseTools.getUserIdSeq = function() {
    return baseTools.getCookie("USER_SEQ");
};
baseTools.getUserDataByKey = function(key) {
    return baseTools.getCookie("QX_USER")[key];
};
