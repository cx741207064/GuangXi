...
(function (factory) {
    ...
    function inEffectiveFields(field, options) {
        return !(field === '__index' ||
            options.effectiveFields.length &&
    ...
    function ajax(options, keyword) {
        ...
        if(options.wingsType == true){
        	//为wings的改造
        	//alert("lei");	
        	//处理参数及额外参数
        	var params = {};
        	params[options.wingskeyParamsName] = keyword;
        	//alert(params[options.wingskeyParamsName]);
        	
	       	if(options.onLtidBefore ){
	       		var tempFn =  wings_getFunc(options.onLtidBefore);
				for (var b = 0; b < tempFn.length; b++) {
		            var  func = eval(tempFn[b]);
			       	if (typeof func == "function") {
		            	var result = new func(params);
			       	}
			    }
			}
	       	var postData = '{"name":"'+options.wingskeyParamsName+'","value":"'+keyword+'","sword":"attr"}';
	       	$.each(params,function(name,value) {
	       		if(name != options.wingskeyParamsName){
	       			postData += ',{"name":"'+name+'","value":"'+value+'","sword":"attr"}';
	       		}
	       	}); 
        	var postData = '{"tid":"","ctrl":"' + AddBizCode2URL(options.wingsCtrl) + '",'
		 		+ '"data":[' + postData +'],'
				+ '"bindParam":true} ' ;
        	
			var url = '/ajax.sword?ctrl='+ options.wingsCtrl;
    ...
    var methods = {
        init: function(options) {
            ...
            options.effectiveFields = options.effectiveFields || [];

            // 默认配置有效显示字段多于一个，则显示列表表头，否则不显示
            if (undefined === options.showHeader && options.effectiveFields.length > 1) {
