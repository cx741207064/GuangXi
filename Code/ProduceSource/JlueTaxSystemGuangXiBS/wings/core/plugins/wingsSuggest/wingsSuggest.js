...
//	                data.value.push({"Id2": i + 1,"Keyword2": valueMapObj["name"].value,"Count2": valueMapObj["id"].value });
		...
		$("#"+opt.id).bsSuggest(sugOpts).on('onDataRequestSuccess', function (e, result) {
	        //console.log('onDataRequestSuccess: ', result);
  				...
  				//执行前方法
  				if(opt.onSelect ){
  					var onSelectFn = wings_getFunc(opt.onSelect);
					for (var b = 0; b < onSelectFn.length; b++) {
			            var  func = eval(onSelectFn[b]);
				       	if (typeof func == "function") {
			            	var result = new func(keyword.key, data);
				       	}
				    }
				};
		...
		//设置值
  		...
  		$suggestObj.setDisable = function(){
  			$suggestObj.disable();
  		}
