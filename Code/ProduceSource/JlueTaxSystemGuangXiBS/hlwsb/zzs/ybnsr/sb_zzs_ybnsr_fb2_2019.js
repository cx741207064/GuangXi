...
var fb2_2019 = (function () {
    ...
    return {
        onExport: function () {
            ...
            curSeg = fb2_2019;
            //引用报表控制
            dcell(DCellWeb1, "/hlwsb/printModel/zzs/ybnsr/sb_zzs_fb2_2019.cll");
            curSeg.onQuery();
        ...
        onPrint: function () {
            ...
            window.location = hlwsbTools.cnf.clientURL+"/hlwsb/zzs_print/ybnsr/sb_zzs_ybnsr_fb2_2019.html?" + str;
        },
        //查询数据
        onQuery: function () {
            ...
            baseTools.xhrAjax({
                url: url,
                params: {
                    SBBZL_DM: SBBZL_DM, SSSQ_Q: SSSQ_Q, SSSQ_Z: SSSQ_Z, OLDSBH: oldsbh,NSRLX_DM: NSRLX_DM
                },
        ...
        checkSave: function () {
            ...
            var col_8se = cell.getValByLabel("G16");
            ...
            var kspzje = cell.getValByLabel("F19");
            var kspzse = cell.getValByLabel("G19");
            var kspzfs = cell.getValByLabel("E19");
            var col09fs = cell.getValByLabel("E18");
            var col09je = cell.getValByLabel("F18");
            var col09se = cell.getValByLabel("G18");
            if(col09fs==0 && col09je==0 && col09se==0){//广西减税降费办公室添加本地需求 20190428 changpengyang
                alert('请确认您本期是否申报抵扣用于构建不动产的扣税凭证，如有申报，则在填报本表第1栏中本期用于购' +
                    '建不动产的增值税专用发票和第4栏中本期用于购建不动产的其他扣税凭证的同时，应将其合计数情况填入“（三）本期用于购建不动产的扣税凭证”栏');
            }
            if(kspzfs<0||kspzje<0||kspzse<0){
                alert('本期用于抵扣的旅客运输服务扣税凭证分数、金额、税额应该大于等于零');
                return;
            }
            if(kspzfs==0 && kspzje==0 && kspzse==0){
                alert('请确认您本期是否申报抵扣旅客运输服务扣税凭证；如有申报，则在填报本表第1栏中按规定本期允许' +
                    '抵扣的购进旅客运输服务取得的增值税专用发票和第4栏中按规定本期允许抵扣的购进旅客运输服务取得' +
                    '的其他扣税凭证的同时，应将其合计数情况填入“（四）本期用于抵扣的旅客运输服务扣税凭证”栏次');
            }
            //新增广西特色需求-监控规则
            ...
            for (var i = 14; i <= 14; i++) {
                ...
                if (Number(jiankongAddCount1) / Number(jiankongAddCount2) > 0.09) {
                    AddCountShowHC = i - 8;
                    alert("第"+AddCountShowHC+"行"+"自2019年4月1日后，纳税人购进农产品，原适用10%扣除率的，扣除率" +
                        "调整为9%。纳税人购进用于生产或者委托加工13%税率货物的农产品，按照10%的扣除率计算进项税额。您填报了适用10%扣除率的税额，请您确认是否填报正确。");
                }
            }
            ...
            //广西本地减税降税办公室提出的本地化需求 lsq 20190428*************
            if ((col_6fs > 0 && col_6je > 0 && col_6se > 0)&&(col_8se==0)) {
                alert('请您确认本期是否发生用于生产或者委托加工13%税率货物的购进农产品，如发生则应填写该栏次“税额”。');
            }
            if (col_8se>0) {
                if (!confirm("农产品进项税额应按1%加计扣除，请您确认是否正确抵扣,是否继续保存？")) {
                    return false;
                }
            }
            if ((col_6fs > 0 && col_6je > 0 && col_6se > 0)&&(col_6se==col_8se)) {
                alert('请您确认本期购进农业生产者自产农产品是否全部领取用于生产或者委托加工13%税率货物。\n');
            }
            //************************************************************
            if (col_35fs < col_26fs) {
            ...
            var col09_se = Number((DCellWeb1.getValByLabel("G18")).toFixed(2));
            var col09_je = Number((DCellWeb1.getValByLabel("F18")).toFixed(2));
            var col10_se = Number((DCellWeb1.getValByLabel("G19")).toFixed(2));
            var col10_je = Number((DCellWeb1.getValByLabel("F19")).toFixed(2));
            var col04_se = Number((DCellWeb1.getValByLabel("G12")).toFixed(2));
            var col04_je = Number((DCellWeb1.getValByLabel("F12")).toFixed(2));
            var col01_se = Number((DCellWeb1.getValByLabel("G9")).toFixed(2));
            var col01_je = Number((DCellWeb1.getValByLabel("F9")).toFixed(2));
            var col01_04_se=Number((col04_se + col01_se).toFixed(2));
            var col01_04_je=Number((col04_je + col01_je).toFixed(2));
            var col09_10_se=Number((col09_se+col10_se).toFixed(2));
            if(col09_se < 0 ){
                alert("第九栏税额应大于等于0");
                return false;
            }
            ...
            //单行小于1+4合计  20190422修改
            if(col09_se >  col01_04_se){
                alert("第九栏税额:" + col09_se + "应小于等于第一栏税额:" + col01_se + "+第四栏税额:" + col04_se + "之和！" );
                return false;
            }
            if(col10_se > col01_04_se){
                alert("第十栏税额:" + col10_se + "应小于等于第一栏税额:" + col01_se + "+第四栏税额:" + col04_se + "之和！" );
                return false;
            }
            if (col09_je > col01_04_je) {
                alert("第九栏金额:" + col09_je + "应小于等于第一栏金额:" + col01_je + "+第四栏金额:" + col04_je + "之和！" );
                return false;
            }
            if (col10_je > col01_04_je) {
                alert("第十栏金额:" + col10_je + "应小于等于第一栏金额:" + col01_je + "+第四栏金额:" + col04_je + "之和！" );
                return false;
            }
            if (col09_10_se > col01_04_se) {
                alert("第九栏税额:" + col09_se + "第十栏税额:"+col10_se+"之和应小于等于第一栏税额:" + col01_se + "+第四栏税额:" + col04_se + "之和！" );
                return false;
            }
        ...
        /*    var col_8bse = cell.getValByLabel("G17");
            ...
            for(i=24;i<=34;i++){
                var jxsezce=cell.getValByLabel("E"+i);//获取进项税额转出额第13-23栏填写数值
                if(jxsezce<0){
                    alert("二、进项税额转出额’第"+(i-11)+"栏不得填写负数!");
                    return false;
                }
            }
            return true;
        ...
        initData: function (jsonObj) {
            ...
            var wxts = "请确认自动带出的发票数据无误后，完成报表填写后保存即可！\r\n" +
                "纳入购进农产品抵扣增值税进项税额试点范围的增值税一般纳税人，销售产品适用13%税率的，在8a栏次“加计扣除农产品进项税额”进行税额加计扣除。";
            if(yqwrdybr=='1'){
                alert("您为逾期未认定一般纳税人，不能填写该表！");
                wxts = wxts + "您为逾期未认定一般纳税人不能填写该表！";
                curSeg.invalidate();
            }
...
//            if (SFFDQ == '201') {
            ...
            * */
            if(SFFDQ.indexOf('202') < 0 && CKTSQYBZ != 'Y'){
                ...
                DCellWeb1.setValByLabel('A3', wxts + "\r待抵扣进项税额部分只能辅导期一般纳税人及出口退税企业填写！");
            }
        },
        ...
        initannotation: function () {
            ...
            cell.setCellannotation('E32', "此栏数据从核心征管提取，不允许修改");
            cell.setCellannotation('E33', "此栏数据从核心征管提取，不允许修改");
            // cell.setCellannotation('G19', "《附列资料五》第3列本期可抵扣不动产进项税额");20190422 胡焕丽让去掉
