...
var fb1_2019 = (function () {
    ...
    return {
        ...
        onLoad: function () {
            curSeg = fb1_2019;
            //引用报表控制
            dcell(DCellWeb1, "/hlwsb/printModel/zzs/ybnsr/sb_zzs_fb1_2019.cll");
            curSeg.onQuery();
            ...
            if((SSSQ_Q == "2018-01-01") && (SSSQ_Z == "2018-01-31"))
            {
                ...
                if(dqsj<"2018-02-04"){
                    window.showModalDialog(hlwsbTools.cnf.clientURL+"/hlwsb/bbsxsm/zzs_fb_ts.html",'',"dialogWidth=500x;dialogHeight=150px;center:yes;status:no;scroll:no;");
                }
        ...
        onPrint: function () {
            ...
            window.location = hlwsbTools.cnf.clientURL+"/hlwsb/zzs_print/ybnsr/sb_zzs_ybnsr_fb1_2019.html?" + str;
        },
        //查询数据
        onQuery: function () {
            ...
            baseTools.xhrAjax({
                url: url,
                params: {
                    SBBZL_DM: SBBZL_DM, SSSQ_Q: SSSQ_Q, SSSQ_Z: SSSQ_Z, YGZNSRLX_DM: YGZNSRLX_DM,NSRLX_DM: NSRLX_DM
                },
        ...
        checkData: function () {
            ...
            var stat = '';
            var col0609 = DCellWeb1.getValByLabel("M13");//20170712 M13-->M14
            var col0109 = DCellWeb1.getValByLabel('M8');
            var col04a09 = DCellWeb1.getValByLabel('M10');
            var hj = curSeg.roundTFix(col0109 + col04a09);
            if (hj >= 0 && col0609 > hj) {
                //20190709 广西需求是第6行第9列小于等于本表第1、3行第9列之和并且当1、3行第9列之和小于0不进行校验
                alert("第6行第9列【" + col0609 + "】不允许大于第1、3行第9列之和【" + hj + "】");//20170712 添加4a
                return true;
            }
            var col0610 = DCellWeb1.getValByLabel('N13');//20170712 错误：（M13-->M14） 正确：（N13-->N14）
            var col0110 = DCellWeb1.getValByLabel('N8');
            var col04a09 = DCellWeb1.getValByLabel('N10');
            var hj = curSeg.roundTFix(col0110 + col04a09);
            if (hj >= 0 && col0610 > hj) {
                //20190709 广西需求是第6行第10列小于等于本表第1、3行第10列之和并且当1、3行第10列之和小于0不进行校验
                alert("第6行第10列【" + col0610 + "】不允许大于第1、3行第10列之和【" + hj + "】");
                return true;

            }
            var col0709 = DCellWeb1.getValByLabel('M14');//20170712 M14-->M15
            var col0209 = DCellWeb1.getValByLabel('M9');
            var col04b09 = DCellWeb1.getValByLabel('M11');
            var col0509 = DCellWeb1.getValByLabel('M12');
            var hj = curSeg.roundTFix(col0209 + col04b09 + col0509);
            if (hj >= 0 && col0709 > hj) {
                //20190709 广西需求是第7行第9列小于等于本表第2、4、5行第9列之和并且当2、4、5行第9列之和小于0不进行校验
                alert("第7行第9列【" + col0709 + "】不允许大于第2、4、5行第9列之和【" + hj + "】");//20170712 4-->4b
                return true;

            }
            var col0710 = DCellWeb1.getValByLabel('N14');
            var col0210 = DCellWeb1.getValByLabel('N9');
            var col04b10 = DCellWeb1.getValByLabel('N11');
            var col0510 = DCellWeb1.getValByLabel('N12');
            var hj = curSeg.roundTFix(col0210 + col04b10 + col0510);
            if (hj >= 0 && col0710 > hj) {
                //20190709 广西需求是第7行第10列小于等于本表第2、4、5行第10列之和并且当2、4、5行第10列之和小于0不进行校验
                alert("第7行第10列【" + col0710 + "】不允许大于第2、4、5行第10列之和【" + hj + "】");
                return true;

            }
            var col0714 = DCellWeb1.getValByLabel('R14');
            var col0214 = DCellWeb1.getValByLabel('R9');
            var col04b14 = DCellWeb1.getValByLabel('R11');
            var col0514 = DCellWeb1.getValByLabel('R12');
            var hj = curSeg.roundTFix(col0214 + col04b14 + col0514);
            if (hj >= 0 && col0714 > hj) {
                //20190709 广西需求是第7行第14列小于等于本表第2、4、5行第14列之和并且当2、4、5行第14列之和小于0不进行校验
                alert("第7行第14列【" + col0714 + "】不允许大于第2、4、5行第14列之和【" + hj + "】");
                return true;
            }

            var col1514 = DCellWeb1.getValByLabel('R25');
            var col1214 = DCellWeb1.getValByLabel('R20');
            var col9b14 = DCellWeb1.getValByLabel('R17');
            var hj = curSeg.roundTFix(col1214 + col9b14);
            if (hj >= 0 && col1514 > hj) {
                //20190709 广西需求是第15行第14列小于等于本表第12、9b行第14列之和并且当12、9b行第14列之和小于0不进行校验
                alert("第15行第14列【" + col1514 + "】不允许大于第12、9b行第14列之和【" + hj + "】");
                return true;
            }
            var col1409 = DCellWeb1.getValByLabel('M24');
            var col0809 = DCellWeb1.getValByLabel('M15');
            var col0909a = DCellWeb1.getValByLabel('M16');
            var col1009 = DCellWeb1.getValByLabel('M18');
            var col1109 = DCellWeb1.getValByLabel('M19');
            var hj = curSeg.roundTFix(col0809 + col0909a + col1009 + col1109);
            if (hj >= 0 && col1409 > hj) {
                //20190709 广西需求是第14行第9列小于等于本表第8、9a、10、11行第19列之和并且当8、9a、10、11行第9列之和小于0不进行校验
                alert("第14行第9列【" + col1409 + "】不允许大于第8、9a、10、11行第9列之和【" + hj + "】");
                return true;
            }

            var col1410 = DCellWeb1.getValByLabel('N24');
            var col0810 = DCellWeb1.getValByLabel('N15');
            var col0910a = DCellWeb1.getValByLabel('N16');
            //var col0910b = DCellWeb1.getValByLabel('N17');
            var col1010 = DCellWeb1.getValByLabel('N18');
            var col1110 = DCellWeb1.getValByLabel('N19');
            var hj = curSeg.roundTFix(col0810 + col0910a + col1010 + col1110);
            if (hj >= 0 && col1410 > hj) {
                //20190709 广西需求是第14行第10列小于等于本表第8、9a、10、11行第10列之和并且当8、9a、10、11行第10列之和小于0不进行校验
                alert("第14行第10列【" + col1410 + "】不允许大于第8、9a、10、11行第10列之和【" + hj + "】");
                return true;
            }

            var col1509 = DCellWeb1.getValByLabel('M25');
            var col1209 = DCellWeb1.getValByLabel('M20');
            var col0909b = DCellWeb1.getValByLabel('M17');
            var hj = curSeg.roundTFix(col1209 + col0909b);
            if (hj >= 0 && col1509 > hj) {
                //20190709 广西需求是第15行第9列小于等于本表第12行、9b行第9列之和并且当12行与9b行第9列之和小于0不进行校验
            ...
            var col1510 = DCellWeb1.getValByLabel('N25');
            var col1210 = DCellWeb1.getValByLabel('N20');
            var col0910b = DCellWeb1.getValByLabel('N17');
            var hj = curSeg.roundTFix(col1210 + col0910b);
            if (hj >= 0 && col1510 > hj) {
                //20190709 广西需求是第15行第10列小于等于本表第12行、9b行第10列之和并且当12行与9b行第10列之和小于0不进行校验
                alert("第15行第10列【" + col1510 + "】不允许大于第12、9b行第10列之和【" + hj + "】");   //20160519王昆松老师确认
                return true;

            }

            var col13a_14 = DCellWeb1.getValByLabel('R21');
            var col13a_13 = DCellWeb1.getValByLabel('Q21');
            var col13b_14 = DCellWeb1.getValByLabel('R22');
            var col13b_13 = DCellWeb1.getValByLabel('Q22');
            var col13c_14 = DCellWeb1.getValByLabel('R23');
            var col13c_13 = DCellWeb1.getValByLabel('Q23');
...
//                DCellWeb1.setValByLabel('F4', "温馨提示：您具备即征即退资格，请注意填写即征即退资格相关行次！");
                ...
                DCellWeb1.SetCellNote(18, 14, 0, ""); //具备即征即退资格,控制第7行14列可写
                if (YGZNSRLX_DM == '22') {
                    flag_6_14 = true;
                    flag_7_15 = true;
                ...
                DCellWeb1.SetCellNote(18, 14, 0, "readonly"); //控制第7行14列是否可写
            }
...
//                if ("Y" == YZL_FLG||Number(YZL)>0) {//
            //2019.6.4 hzj 广西地方特色要求 （与总局需求冲突）13a与13c填写:添加规则为接口报文中tlysqybz=“Y” 与13b判断条件相似。
            if (YGZNSRLX_DM == '20' || YGZNSRLX_DM == '22') {
                if (YBNSRYZSKDFPBZ == "Y" || YZL_FLG == "Y") {
                    yzl_row.push([21]);
                    if (parseFloat(YZL) > 0.00) {
                        DCellWeb1.setValByLabel('C21', "预征率" + (parseFloat(YZL) * 100).toFixed(4) + "%"); //2014.5.21lijun
                ...
                if (YBNSRYZSKDFPBZ == 'Y') {//YBNSRHZJNZG='F' 没取值
                    ...
                    if (parseFloat(YBNSRFZJGDFPYZL) > 0.00) {
                        DCellWeb1.setValByLabel('C22', "预征率" + (parseFloat(YBNSRFZJGDFPYZL) * 100).toFixed(4) + "%"); //2014.5.21lijun
                        DCellWeb1.DefineDoubleVar("YZLB", parseFloat(YBNSRFZJGDFPYZL));
                        DCellWeb1.setValByName("13B", YZLB);
                    }
                }

                if (YBNSRYZSKDFPBZ == 'Y' || YBNSRZCDTLZG == 'Y') {//YBNSRHZJNZG='F'
                    DCellWeb1.SetCellInput(3, 23, 0, 7);
                    yzl_row.push([23]);
                    if (parseFloat(YZLC) > 0.00) {
                        DCellWeb1.DefineDoubleVar("YZLC", parseFloat(YZLC));
                        DCellWeb1.setValByLabel('C23', "预征率" + (parseFloat(YZLC) * 100).toFixed(4) + "%");
            ...
            for (var i = 0; i < yzl_row.length; i++) {
                if (yzl_row[i] == 21) {
                    tip += '13a,';
                } else if (yzl_row[i] == 22) {
                    tip += '13b,';
                    DCellWeb1.SetCellInput(18, 22, 0, 2);//测试部要求将14列改为自动计算可修改
            ...
            if (YGZNSRLX_DM == '10') {// 10-非营改增纳税人
                ...
                DCellWeb1.setValByLabel('F3', "温馨提示：您是非营改增纳税人，可以填写1,3,8,9a,10,11," + tip + "16,18行");
                //nsrlx_row = [8, 10, 11, 16, 17, 19, 20, 27, 29]
                nsrlx_row = [8, 10, 15, 16, 18, 19, 26, 28]
                //nsrlx_row = [8,10,19,26,28];
                curSeg.setReadOnlyByLabel(5, 20, (nsrlx_row.concat(yzl_row)).concat(extra_row));
                //单独锁死第18列
                DCellWeb1.SetCellInput(18,9,0,5);
                DCellWeb1.SetCellInput(18,11,0,5);
            } else if (YGZNSRLX_DM == '20') { //20-营改增纳税人

                DCellWeb1.setValByLabel('F3', "温馨提示：您是营改增纳税人，可以填写2,4,5,9b,12," + tip + "17,19行");
                nsrlx_row = [9, 11, 12, 17, 20, 27, 29];
                curSeg.setReadOnlyByLabel(5, 20, (nsrlx_row.concat(yzl_row)).concat(extra_row));
            } else if (YGZNSRLX_DM == '22') {           //兼营22，32
                DCellWeb1.setValByLabel('F3', "温馨提示：您是兼营的纳税人，可以填写1,2,3,4,5,8,9a,9b,10,11,12," + tip + "16,17,18,19");        //201605192004王昆松要求8，9a,10除营改增外都放开
                //DCellWeb1.setValByLabel('F3',"温馨提示：您是混营的纳税人，如果本期既有应税货物及劳务又有应税服务，因系统故障！请您延迟申报（2015年1月16日后再行申报）");
                nsrlx_row = [8, 9, 10, 11, 12, 15, 16, 17, 18, 19, 20, 26, 27, 28, 29];
                curSeg.setReadOnlyByLabel(5, 20, (nsrlx_row.concat(yzl_row)).concat(extra_row));
            }
                ...
                if (col == 3 && (row == 23)) {
                    ...
                    if (0 < parseFloat(Number(text)) && parseFloat(Number(text)) < 1) {
                        window.setTimeout(function () {
                            DCellWeb1.setValByLabel('23', '预征率' + (parseFloat(Number(text)) * 100).toFixed(4) + '%');
                        });
                        return true;
                    } else {
            ...
            if (yzl_row[0] == 21) {
                DCellWeb1.SetCellInput(18, 21, 0, 2);
            }
            ...
            if(JYZSBFBZ != '203'){
                for(var i = 15;i <= 25;i++){
                    for(var j = 5;j <= 18;j++){
                        ...
                        DCellWeb1.SetCellBackColor(j, i, 0,DCellWeb1.FindColorIndex(0xEBEBEB,1));
                    }
                }
            ...
            for (var i = 0; i < rows.length; i++) {
                for (var j = startcol; j < endcol; j++) {
                    ...
                    if (DCellWeb1.GetCellString(j, rows[i], 0) != '——' && type != 'readonly') {
                        ...
                        if( ((5<=j<=8) && rows[i]<=20 && rows[i]!=14 && rows[i]!=13 && rows[i]!=15) || rows[i]==28  || rows[i]==29 ){
                            if(rows[i]!=29){
                                DCellWeb1.SetCellColor("E"+rows[i], 2);
                                DCellWeb1.SetCellColor("F"+rows[i], 2);
                                DCellWeb1.SetCellTip(5,rows[i],0,"该数据来自于税控系统，如发现有误，可自行修改！");
                                DCellWeb1.SetCellTip(6,rows[i],0,"该数据来自于税控系统，如发现有误，可自行修改！");
                            }
                            DCellWeb1.SetCellColor("G"+rows[i], 2);
                            DCellWeb1.SetCellTip(7,rows[i],0,"该数据来自于税控系统，如发现有误，可自行修改！");
                            if(rows[i]!=28 && rows[i]!=29){
                                DCellWeb1.SetCellColor("H"+rows[i], 2);
...
//			window.open("sb_zzs_fp_hz.html?SBBZL_DM="+SBBZL_DM,'','height=400,innerHeight=400,width=660,innerWidth=680,top='+iTop+',left='+iLeft);
        ...
        openPopWin: function () {
            ...
            var winParam = {
                ...
                url: '/hlwsb/components/excelToJson/sb_excel_upload.html?bbmc=fb1_2019&&startRow=9&&sheetIndex=1&templatePath=printModel/zzs/ybnsr/sb_zzs_fb1_2019.zip',
                width: 400,
                height: 150
            };
            winObj = baseTools.showWinExt(winParam);
