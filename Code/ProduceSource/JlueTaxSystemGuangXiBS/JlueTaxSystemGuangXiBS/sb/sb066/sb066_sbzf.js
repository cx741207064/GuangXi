...
function init_sbzf(resData){
	 ...
	 * begin
	 */
	viewSbbBz ="Y";// resData.getAttr("viewSbbBz");//网报特色修改：查看申报表标志
...
function queryBynsrsbhSuccess(nsrParam,zrrParam){
	...
	ssqzBtn.setCtrl("SB066SbxxcxCtrl_querySbbzl");
...
function checkYzpzzl() {
	...
	changeSbbzlBtn.setCtrl("SB066SbxxcxCtrl_getSkssqzByYzpzzlDm");
...
function querySbb(){
	...
	if(checkNull(pzxh)||checkNull(yzpzzlDm)||checkNull(sbrq1)||checkNull(skssqq)||checkNull(skssqz)||checkNull(zsxmDm)){
		...
		cxBtn.setCtrl("SB066SbxxcxCtrl_querySbbxx");
...
function viewSbb(){
	...
	if(checkRowData.trs.length==0){
		...
		var origin=checkRowData.trs[0].tds.origin.value;
		//网报特殊调整，新版房土不支持申报表查看
		if("BDA0610790"==yzpzzlDm){
			...
			sbbCkInitViewGyCtrl(djxh,pzxh,sbuuid,skssqq,skssqz,sbrq,yzpzzlDm,sbsxDm1,origin);
		}
	}
}
...
function sbbCkInitViewGyCtrl(djxh,pzxh,sbuuid,skssqq,skssqz,sbrq,yzpzzlDm,sbsxDm1,origin){
	if($chk(origin)&&"gs"==origin){
		// gsdz = gsdz +"/sword?ctrl=GY007SbSbbdxxCkCtrl_initViewGds";
		//window.location.href 
		
		var url = gsdz+"/sword?ctrl=DZBSXTKJ022YhmMmDlCtrl_guXLocalTaxWinOpen&functionName="+
							encodeURIComponent("申报查看")+"&functionUrl="+encodeURIComponent(gsdz+"/sword?ctrl=GY007SbSbbdxxCkCtrl_initViewGds" +"?pzxh="+pzxh+
		"&sbuuid="+sbuuid+
		"&djxh="+djxh+
		"&yzpzzlDm="+yzpzzlDm+
		"&scenceCs=sbbck")+"&functionType=01";
//		return;
//		swordAlertIframe(url,{width:1380,height:900});   
		window.open(url);
	}else{
		...
		serviceForwardBtn.setCtrl("GY007SbSbbdxxCkCtrl_initViewGds");
		swordAlertIframe('', {
				titleName : "查看申报表",
				param : window,
				isMax : 'true',
...
function doExport(){   
    ...
    sub.setCtrl(gt3_ctrl + "_export");   
    sub.pushData($w('sbsjxxGrid').getAllGridData());     
    sub.submit();
} 