(function($){
	...
	$.fn.wingsTab= function(options){
		...
		if(opts.isColumn == "true"){
			//设置成纵向的tab
			$tabObj.addClass("wings_tab_s0");
		}
...
//		var tempLiId = opts.name+"_wingsTab_a"+  Math.floor(Math.random()*100);
		...
		for(var i = 0;i< opts.divIds.length;i++){
			...
			if(onSelectfn || opts.onselect!=null){
				var lastIndex = i;
				//追加方法
				$("#"+opts.name+'_wingsTab_a_'+opts.divIds[i]).click(function(e){
					$tabObj.onSelect($("#" + this.id).attr('mun') );
					//整体onSelect方法
					if(opts.onselect!=null){
						var index = $(e.target).attr('mun');
						var tempFn =  wings_getFunc(opts.onselect);
						for (var b = 0; b < tempFn.length; b++) {
				            var  func = eval(tempFn[b]);
					       	if (typeof func == "function") {
				            	var result = new func($tabObj, $tabObj.lastIndex ,index);
					       	}
					    };
						$tabObj.lastIndex = index;
					}
		...
		//获得哪个tab页签是当前选中的
		$tabObj.getActTabName = function(){
			var str = '';
			$('#'+opts.the_tab_id).find("li.active").each(function(j,el){
				str = el.id;
				str = str.replace(opts.name+'_wingsTab_li_','');
			});
			return str;
		}
