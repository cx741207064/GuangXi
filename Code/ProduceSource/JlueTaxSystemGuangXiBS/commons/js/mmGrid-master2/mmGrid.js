        ...
        , _rowHtml: function(item, rowIndex){
            ...
            if($.isPlainObject(item)){
                ...
                for(var colIndex=0; colIndex < leafCols.length; colIndex++){
                    ...
                    trHtml.push('"><span style="+display:block; +width:100%" class="');
                    if(opts.nowrap){
                        trHtml.push('nowrap');
                    }
                    trHtml.push('">');
        ...
        , select: function(args){
            ...
            if(typeof args === 'number'){
                ...
                if(!$tr.hasClass('selected')){
                    ...
                    if(opts.checkCol){
                         var $checkbox =   $tr.find('td .mmg-check');
                ...
                $.each($body.find('tr'), function(index){
                    if(args($.data(this, 'item'), index)){
                        ...
                        if(!$this.hasClass('selected')){
                            ...
                            if(opts.checkCol){
                                var $checkbox =  $this.find('td .mmg-check');
                    			if($checkbox.prop('disabled')!=true)
                       				$checkbox.prop('checked','checked');
                            }
                        }
                ...
                var $checkbox =  $body.find('tr > td').find('.mmg-check');
                if($checkbox.prop('disabled')!=true)
                       	$checkbox.prop('checked','checked');
