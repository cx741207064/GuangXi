...
var fykg = "N"; // 分页参数
var zzlx = "A4";// 纸张参数
var printMode = '{"PRINT_PAGE_PERCENT":"90%"}';// 缩放参数
var printTitle = "通用申报表（税及附征税费）"; // 模版标题
var gzsekg;// 更正开关
var ybtseyz;// 原应补退税额
var xwqyjmtskg = "N";//网报特色改造，小微企业减免提示开关
var xwqybz = "N";//小微企业标志
var wglbz = "N";//未归类标志
/**
 * 主表打印函数
...
function setZbdymbuuid() {
	...
	printMode = '{"PRINT_PAGE_PERCENT":"90%"}';
	printTitle = "通用申报表（税及附征税费）";
	return dymbuuid;
}
function printhtm() {
	var swjgTemp= $w("slxxForm").getValue("bsr");
	if ("Y" == htmbfzkg) {
		gyTaxPrintWordOrExcel();
	} else {
		$w("slxxForm").setValue("slswjg", slswjgMc);
		gyTaxPrint();
		$w("slxxForm").setValue("slswjg", swjgTemp);
	}
}
/*******************************************************************************
 * 初始化页面
...
function init(resData) {
	//填表说明
	jspt_czsm_butncontrol("fxmtysbToolBar","SB175");	
	gzsekg = resData.getAttr("gzsekg"); //网报更正税额开关
	htmbfzkg = resData.getAttr("htmbfzkg"); // 网报
	xwqyjmtskg = resData.getAttr("xwqyjmtskg");//网报
	slswjgMc= resData.getAttr("slswjgMc");
	...
	if ($chk(resData.getAttr("scenceCs"))
			&& "sbbck" == resData.getAttr("scenceCs")) {
		gt3.sbbck = "Y";
	}
	...
	(function() {
		...
		var cxbz = "4";
		queryNsrxxbyNsrsbh(nsrxxForm.getValue("nsrsbh"), queryNsrxxSuccess,
				cxbz);
	}).delay(1);
...
// 设置grid从价/量的减免税额、税率是否可编辑
function setGridCellEditable() {
	var form = $w("nsrxxForm");
	var ssqq = form.getValue("skssqq");// 税款所属期起
	var ssqz = form.getValue("skssqz");// 税款所属期止
	var cells = $w('skxxGrid').getCells('sfkssqq');
	cells.each(function(theEl, index) {
		$w('skxxGrid').cellDisable(theEl);
	});
	var cells1 = $w('skxxGrid').getCells('sfkssqz');
	cells1.each(function(theEl, index) {
		$w('skxxGrid').cellDisable(theEl);
	});
}
...
function queryNsrxxSuccess(nsrParam, zrrParam) {
	...
	if (nsrParam != "" && nsrParam != null) {
		...
		// 判断当前纳税人是否是报验户
		var kzztdjlxDm = nsrParam.kzztdjlxDm.value;
		if ($chk(kzztdjlxDm) && kzztdjlxDm == '1140') {
			gt3.byhBz = 'Y';
		} else {
			gt3.byhBz = 'N';
		}
	...
	// 检查是否存在自查补报通知书
	checkNsrZxbzTzs(gt3.djxh, nsrxxForm.getValue('skssqq'), nsrxxForm
			.getValue('skssqz'));
...
function changeSSq(inputEl) {
	...
	if (sybz == 1 || sybz == 4) {
		...
		if ($chk(msg)) {
			swordAlert(msg, {
				onOk : function() {
					skxxGrid.setCellFocus(row.getCell("xh").get("realvalue"),
							elName);// 光标回到出发控件
				}
			});
...
function initData() {
	...
	if("Y"==xwqyjmtskg){//内蒙地税需求，对未划形企业提示纳税人做归类管理。
		queryDjhglxx();
		if("Y" == wglbz){
	...
	if (gt3.scenceCs == "cwgzbz" && "Y" != cxbj && gt3.sbbck != "Y") { // 20160922网报调整
		...
		for (var i = 0; i < rowDatas.length; i++) {
			...
			if (zsxmDm == "10103" || zsxmDm == "10101" || zsxmDm == "10102"
					|| zsxmDm == "30446" || zsxmDm == "30138"
					|| zsxmDm == "30221" || zsxmDm == "30126"
					|| zsxmDm == "30217") {
				var mzed = 0;
				var sfkssqq = rowData.getValue("sfkssqq");
				var sfkssqz = rowData.getValue("sfkssqz");
 ...
 * 获取税费种信息（按钮触发）
 */
function setSkxx(flag) { // 网报增加flag 用于区分是否初始化自动调用
	var nsrxxForm = $w("nsrxxForm");
	var skssqq = nsrxxForm.getValue("skssqq");// 所属期起
	var skssqz = nsrxxForm.getValue("skssqz");// 所属期止
	// 不存在税种信息，给出 提示
	...
	for (var j = 0; j < gt3.skxxList.trs.length; j++) {
		...
		var kssqq = gt3.skxxList.trs[j].tds.sfkssqq.value;
		var kssqz = gt3.skxxList.trs[j].tds.sfkssqz.value;
		if (!($chk(zspmDm) && ("101016604" == zspmDm || "101016606" == zspmDm))) {
			var row = skxxGrid.insertRow(gt3.skxxList.trs[j]);
			// 网报特色 属期置灰
			var sfkssqqCell = row.getElement('[name=sfkssqq]');
			var sfkssqzCell = row.getElement('[name=sfkssqz]');
			var dzswj_sqkzkg = isEnableSkssq("BDA0610100");
			if ($chk(dzswj_sqkzkg) && dzswj_sqkzkg == "Y") {
				sfkssqqCell.disable = "true";
				sfkssqzCell.disable = "true";
			} 
			// 非自然人，非报验户，税费种认定的征收项目、品目、子目不能修改
			if ($chk(gt3.byhBz) && 'N' == gt3.byhBz) {
			...
			if ("Y" == gt3.cfsbjybz) {
				...
				if ("Y" == sfysb) {
					...
					skxxGrid.updateCell(row.getCell("zfBtn"), '点击进行申报作废');
					skxxGrid.updateCell(row.getCell("sfysb"), 'Y');
...
function calTable(grid, row, rowData, bz, cshbz) {
	...
	if (bz == 1
		...
		// if (zsxmDm == '10109' || zsxmDm == '30203' || zsxmDm == '30127' ||
		// zsxmDm ==
		// '30216') {
		// ysx = getZsSe(rowData);
		// } else if (zsxmDm == '30126' || zsxmDm == "30217" || zsxmDm ==
		// '30446') {
		// // 文化事业建设费或堤围费,取主税计税依据
		// ysx = getZsYsx(rowData);
		// }
	...
	if (zsxmDm == "10106" && (jsbz == "2" || jsbz == "3")) {
		...
		// 计税（费）依据=（应税项-减除项）*应税所得率
		var jsfyj = isPositive((ysx).subtract(jcx).multiple(yssdl).round(2)) / 1;
	}
	...
	if (sybz == 1) {
		bqynsfe = (jsfyj).multiple(sflhdwse).divide(12).multiple(qs).subtract(
				sskcs).round(2);
	} else if (sybz == 4) {// 房屋
		// 减除率不为空，从价计征
		if (jcl > 0) {
	...
	// 堤围费在根据主税应税项变化后联动时，需要考虑堤围费起征点
	if ((bz != 4 && bz != 5 && (zsxmDm == "10103" || zsxmDm == "10101"
			|| zsxmDm == "10102" || zsxmDm == "30126" || zsxmDm == "30217"
			|| zsxmDm == "30446" || zsxmDm == "30138" || zsxmDm == "30221" || zsxmDm == '10106'))
			|| (bz == 4 && (zsxmDm == "30126" || zsxmDm == "30217"
					|| zsxmDm == "30446" || zsxmDm == "30138" || zsxmDm == "30221"))) {
		var arr = new Array();
		var ysxjHj = getYsxHj(rowData)// 应税项合计
		// 判断页面起征点信息是否有该属期数据
		if (checkQzdxx(rowData)) {
		...
		if (ysxjHj <= mzed) {
			...
			if ($chk(zspmDm)
					&& ("101016604" == zspmDm || "101016606" == zspmDm || "101060801" == zspmDm)) {
			...
			if (bz != 3 && bz != 5) {
				...
				if (ysx / 1 > 0 && ysbje > 0 && mzed > 0 && bz != 1 && bz != 6
						&& sfxswdqzdjm == "Y") {
					swordAlert(row.getCell("zsxmDm").title
							+ "首次达到起征点，完成本次申报后，通过申报错误更正功能追缴同期享受减免的税款,需追缴的税款信息为:"
							+ mesg);
				}
				// TODO 添加减免10w
				var allRows = grid.dataDiv().getChildren();// 所有行
				...
				for (var i = 0; i < length; i++) {
					...
					if ($chk(rdpzuuid) && sfkssqq == sfkssqq2
							&& zsxmDm != "30446"
							&& fsqzdxx >= (ysxjHj / 1 + ysbje / 1)
							&& sfkssqz == sfkssqz2 && rdpzuuid == rdzsuuid) {
						var bqjmsfecell = allRows[i].getCell("bqjmsfe");// 应税项
						var bqybtsfecell = allRows[i].getCell("bqybtsfe");// 应税项
						if (bqybtsfe / 1 > 0) {
	...
	if (yhsl != 0) {
		...
		if ($chk(zszmDm) && zszmDm != "null" && zszmDm != "NULL"
				|| $chk(ssjmxzDm)) {
			yhje = isPositive(jsfyj.multiple((sflhdwse).subtract(yhsl))
					.round(2));
			;
		}
	}
	...
	if (bqjmsfe == 0 && bqynsfe > 0) {// 减免税费额等于0的情况
		var arr = processTsjmxx(gt3.djxh, zsxmDm, zspmDm, sfkssqq, sfkssqz,
				jsfyj, ysx, bqynsfe, bqjmsfe, bqyjsfe, (bqynsfe).subtract(
						bqjmsfe).subtract(bqyjsfe), sflhdwse);
		bqjmsfe = arr[0] / 1;
...
//	}
	...
	if (zsxmDm == "10111") {
		grid.updateCell(row.getCell("bqybtsfe"),
				jsyhsje(zspmDm, (bqybtsfe / 1)));// 更新本期应补退税额
	} else {
		...
		for (var i = 0; i < length; i++) {
			...
			if ($chk(sybh) && sybh == sybh2 && sfkssqq == sfkssqq2
				...
				if (parseFjsxx(zsxmDm2, zspmDm2) == "1") {
					...
					// } else if (zsxmDm2 == "30446" || zsxmDm2 == "30126" ||
					// zsxmDm2 == "30217"
				...
				if ((parseFjsxx(zsxmDm2, zspmDm2) == "1")
						&& rdpzuuid == rdzsuuid) {
					// if ((zsxmDm2 == '10109' || zsxmDm2 == '30203' || zsxmDm2
					// == '30127' ||
					// zsxmDm2 == '30216')
					// && rdpzuuid == rdzsuuid) {
					// 初始化进入、且附税应税项不为0时，不计算
					...
					// } else if (((zsxmDm2 == "30446" || zsxmDm2 == "30126" ||
					// zsxmDm2 == "30217"
					// || zsxmDm2 == "30138" || zsxmDm2 == "30221") && rdpzuuid
					// == rdzsuuid)
					// || ((zspmDm.substring(0, 7) == "1010306" && (zspmDm2 ==
					// "301260100" ||
					// zspmDm2 == "302170100")) || ((zspmDm2 == "301260200" ||
					// zspmDm2 ==
	...
	//地方水利建设基金小微企业减免提示
	var jmxzDm = rowData.getValue("ssjmxzDm");
	var jsyj = rowData.getValue("ysx");
	var skssqq = rowData.getValue("sfkssqq");
	var skssqz = rowData.getValue("sfkssqz");
	xwqyJmts(zspmDm, jmxzDm, jsyj, bqybtsfe, skssqq, skssqz, zsxmDm);
...
function dealWithGrczfw(checkrowData, checkrow, flag) {
	...
	if ("101060801" == zspmDm) {
		...
		for (var i = 0; i < rowDatas.length; i++) {
			...
			if ($chk(zspmDmingr)
					&& ("101016604" == zspmDmingr || "101016606" == zspmDmingr)) {
				var zzsysx = rowData.getValue("ysx");
				var zzsybtse = rowData.getValue("bqybtsfe");
				if ("Y" == flag) {
					ysx = (parseFloat(zzsysx) - parseFloat(zzsybtse))
							.toFixed(2);
				}
				break;
			}
		...
		var ybtse = (parseFloat(ynse) - parseFloat(jmse) - parseFloat(bqyjsfe))
				.toFixed(2);
	...
	btn
			.setOptions({
				'onSuccess' : function(req, res) {
					...
					var ybtse = (parseFloat(ynse) - parseFloat(jmse)
							- parseFloat(bqyjsfe) + parseFloat(xdsje) - parseFloat(bdsje))
							.toFixed(2);
					...
					var qtgrczbdczjsfft = $w("nsrxxForm").getValue(
							"qtgrczbdczjsfft");
					var qtgrczbdczlqq = $w("nsrxxForm").getValue(
							"qtgrczbdczlqq");
					var qtgrczbdczlqz = $w("nsrxxForm").getValue(
							"qtgrczbdczlqz");
					...
					if ($chk(qtgrczbdczjsfft) && $chk(qtgrczbdczjsfft.code)) {
						if ("N" == qtgrczbdczjsfft.code) {
							...
							var num = parseInt(zmonth, 10)
									- parseInt(qmonth, 10) + 1;
					...
					var rows = grid.dataDiv()
							.getChildren('div[status!=delete]');
					var rowDatas = grid.getAllNoDeleteGridData().trs;
					for (var i = 0; i < rowDatas.length; i++) {
						var rowData = rowDatas[i];
						var row = rows[i];
...
function doZzsfz(ysx) {
	...
	for (var i = 0; i < rowDatas.length; i++) {
		...
		if ($chk(gt3.skselbz) && gt3.skselbz == 'N') {
			var zszmDm = rowData.tds.zszmDm.value;
			if ((zszmDm == '160204' || zszmDm == '880103' || zszmDm == '610102'
					|| zszmDm == '160304' || zszmDm == '160104')
					&& ysxValue == 0) {
...
function changeYjse(realvalue, showvalue, rowData, el, row, oldvalue) {
	...
	if (!$chk(yjxxList) || yjxxList.trs.length == 0) {
		...
		yjxxList.trs.each(function(yjxx, index) {
			...
			if (zspmDm == yjZspmDm
					&& (compareDate(yjSkssqq, skssqz) || compareDate(skssqq,
							yjSkssqz))) {
				kyyjse += yjye;
			}
		});
		if (gt3.scenceCs == "cwgzbz" && $chk(skxxGridfromgz)
				&& skxxGridfromgz.trs.length > 0) {
	...
	if ("Y" != gt3.zrrBz) {
		...
		if ($chk(zspmDm) && ("101016604" == zspmDm || "101016606" == zspmDm)) {
			...
			for (var i = 0; i < allData.trs.length; i++) {
				...
				if ($chk(zspming)
						&& $chk(zspming.value)
						&& ("101016604" == zspming.value || "101016606" == zspming.value)) {
					num = parseInt(num) + 1;
					if (num > 1) {
	...
	if (!$chk(sybz)) {
		...
		for (var i = 0; i < skxxList.length; i++) {
			...
			if (!$chk(skZszmDm) && skZsxmDm == zsxmDm && zspmDm == skZspmDm
				...
				checkCfYqSb(skxxGridData);
				// 根据主税信息，添加附税行
				toAddRowFsxxByZsxx(skxxGrid.getCheckedRowData());
				return;
			}
		}
		...
		if ($chk(dzswj_sqkzkg) && dzswj_sqkzkg == "Y") {
			...
			skxxGrid.updateCell(row.getCell("sfkssqq"), sfkssqq);
			skxxGrid.updateCell(row.getCell("sfkssqz"), sfkssqz);
			sfkssqqCell.disable = "false";
			sfkssqzCell.disable = "false";
		}

		skxxGrid.updateCell(row.getCell("nsqxDm"), nsqxDm);
		// 根据主税信息，添加附税行
		toAddRowFsxxByZsxxAndCsb(skxxGrid.getCheckedRowData());
	}
	skxxGrid.updateCell(row.getCell("bqyjsfe"), "0.00");
	// 获取税率
	getHdslHSlxx(row, skxxGrid.getCheckedRowData());
				...
				onOk : function() {
					...
					for (var j = 0; j < fjsList.length; j++) {
						...
						if (row.getCell("ysx").realvalue / 1 > 0) {
							calTable(skxxGrid, row,
									skxxGrid.getOneRowData(row), 4);
						}
					}
				},
				width : 500,
	...
	slBtn
			.setOptions({
				"onSuccess" : function(req, res) {
					...
					arr.push(sl, yhsl, yssdl, sskcs, jsyj, jsbz, ysbse, ynse,
							dqjsFlag);
				}
			});
	slBtn.submit();
		...
		if (zsZsxmDm == "10101") {
			gt3.zspmData
					.each(function(item, index) {
						if (item.pcode == zsxmDm
								&& (item.code == "101090101"
										|| item.code == "101090201" || item.code == "101090201"))
							newData[newData.length] = item;
					});
			return newData;
			...
			gt3.zspmData
					.each(function(item, index) {
						if (item.pcode == zsxmDm
								&& (item.code == "101090102"
										|| item.code == "101090202" || item.code == "101090202"))
							newData[newData.length] = item;
					});
			return newData;
			...
			gt3.zspmData
					.each(function(item, index) {
						if (item.pcode == zsxmDm
								&& (item.code == "101090103"
										|| item.code == "101090203" || item.code == "101090203"))
							newData[newData.length] = item;
					});
			return newData;
		}
...
function declare() {
	var skxxGrid = $w("skxxGrid");
	// 内蒙需求地方教育费附加/教育费附加/地方水利建设基金必须选择减免性质
	if ("Y" == xwqyjmtskg) {
		var fjsfsbxxGrid = skxxGrid.getAllNoDeleteGridData();
		for ( var i = 0; i < fjsfsbxxGrid.trs.length; i++) {
			...
			var bqybtse = fjsfsbxxGrid.trs[i].tds.bqybtsfe.value;
			var jmxzDm = fjsfsbxxGrid.trs[i].tds.ssjmxzDm == null ? ""
					: fjsfsbxxGrid.trs[i].tds.ssjmxzDm.value;
	...
	if (gzsekg == "Y" && gt3.scenceCs == "cwgzbz") {// 申报错误更正只昜大不更小 内蒙地税需求
		// 2018-05-14
		var ynsehj = skxxGrid.getHj("bqybtsfe");
...
function save() {
	...
	if (gt3.scenceCs == "cwgzbz") {
		if (saveBeforeCheck()) {
			...
			saveBtn.pushData("zgswskfjDm", $w("nsrxxForm").getValue(
					"zgswskfjDm"));
		...
		if (saveBeforeCheck()) {
			...
			if ("Y" == gt3.cfsbjybz) {
				var nameCells = $w("skxxGrid").getCells("sfysb");
				nameCells.each(function(el, index) {
					var rowEl = el.parentNode;
					// var status = rowEl.get('status');
					// var rowData = grid.getOneRowData(el);
					var sfysb = el.get("realvalue");
					if ("Y" == sfysb) {
						rowEl.getElement('input[type=checkbox]').set('checked',
								false); // 设为不选中状态
					} else {
						rowEl.getElement('input[type=checkbox]').set('checked',
								true); // 设为选中状态
				...
				skxxGrid = $w("skxxGrid").getCheckedData("xz");
			}
...
function saveSuccess(req, resData) {
	...
	if (gt3.scenceCs != 'cwgzbz') {
		...
		saveSxslxx(returnVO.data.pzxh.value, gt3.djxh, nsrxxForm
				.getValue('nsrsbh'), nsrxxForm.getValue('nsrmc'),
				'SLSXA061001020', 'LCSXA061001019', swjgDm, flzlGridData);
	}
}
...
function mouseMove() {
	...
	grid.eDelegator.add('mouseenter', function(e, defEl) {
		...
		var rowEls = grid.dataDiv().getElements("div.sGrid_data_row_div");
	...
	grid.eDelegator.add('mouseleave',
			function(e, defEl) {
				var sybh1 = grid.getOneRowData(defEl).getValue("sybh1");
				if (!$chk(sybh1)) {
					return;
				...
				var rowEls = grid.dataDiv().getElements(
						"div.sGrid_data_row_div_hover");
				var cclass = "sGrid_data_row_div_hover";
				var len = rowEls.length;
				for (var i = 0; i < len; i++) {
					var sybh1val = grid.getOneRowData(rowEls[i]).getValue(
							"sybh1");
					if (sybh1 == sybh1val) {
						rowEls[i].removeClass(cclass);
					}
	...
	if (zsxmDm == "10103" || zsxmDm == '10109' || zsxmDm == '30203'
		...
		for (var i = 0; i < len; i++) {
			...
			if (sybh1 == sybh2
				...
				if (rowEl.getCell("xz").getFirst("input[type=checkbox]").get(
						"checked")) {
	...
	for (var i = 0; i < rowDatas.length; i++) {
		...
		if (bz == 7 && zsZsxmDm == "10101" || zsZsxmDm == "10102"
			...
			if ($chk(sybh) && sybh == sybh2 && sfkssqq == zsSkssqq
					&& sfkssqz == zsSkssqz) {
				if (zsxmDm2 == "30446" || zsxmDm2 == "30126"
						|| zsxmDm2 == "30217" || zsxmDm2 == "30138"
						|| zsxmDm2 == "30221") {
					// 文化事业建设费、堤围费、地方水利建设基金
					grid.updateCell(ysxCell, emptyToZ(ysx));// 为主税应税项
				...
				if (((zsxmDm2 == "30446" || zsxmDm2 == "30126"
						|| zsxmDm2 == "30217" || zsxmDm2 == "30138" || zsxmDm2 == "30221") && rdpzuuid == rdzsuuid2)
						|| ((zspmDm.substring(0, 7) == "1010306" && (zspmDm2 == "301260100" || zspmDm2 == "302170100")) || ((zspmDm2 == "301260200" || zspmDm2 == "302170200") && zspmDm == "101030708"))) {
					// 堤围费、文化事业建设费、地方水利建设基金
					// 需要对营业税（其他娱乐业、广告业）时，同时更新附税文化事业建设费的联动信息（娱乐业文化事业建设费，广告业文化事业建设费）
					grid.updateCell(ysxCell, emptyToZ(ysx));// 为主税应税项
		...
		if (!$chk(sybh)) {
			...
			if (zsxmDm == "10103" || zsxmDm == "10101") {
				...
				} else if ((parseFjsxx(zsxmDm2, zspmDm2) == "1")
				// (zsxmDm2 == "10109" || zsxmDm2 == "30127" || zsxmDm2 ==
				// '30216' || zsxmDm2 ==
				// "30203" || zsxmDm2 == "30199")
			...
			if (zsxmDm == "10103" || zsxmDm == "10101") {
				...
				} else if ((zsxmDm2 == "10109" || zsxmDm2 == "30127"
						|| zsxmDm2 == '30216' || zsxmDm2 == "30203")
						&& zsSkssqq == sfkssqq && zsSkssqz == sfkssqz) {
					grid.updateCell(rows[i].getCell("ysx"), "0.00");
					grid.updateCell(rows[i].getCell("jcx"), "0.00");
					grid.updateCell(rows[i].getCell("jsfyj"), "0.00");
		...
		if (zsxmDm == zsxmDm2 && zsSkssqq == sfkssqq && zsSkssqz == sfkssqz) {
			if (gt3.scenceCs == "cwgzbz") {// 错误更正时直接赋值为0
				...
				if ($chk(cshbz)
						&& cshbz == "1"
						&& $chk(ssjmxzDm)
						&& "0003045301,0003045302,0003042802,0003045303"
								.indexOf(ssjmxzDm) == -1) {
				} else {
				...
				grid.updateCell(rows[i].getCell("bqjmsfe"), 0.0);
				// grid.updateCell(rows[i].getCell("bqjmsfe"),
				// (bqjmsfe/1).round(2));
...
function getQzdZsxm(rowData) {
	...
	if (zfsbz != "0") {
		...
		// 娱乐业文化事业建设费,取营业税的起征点
		if ((zsxmDm == "30126" || zsxmDm == "30217")
				&& (zspmDm == "301260100" || zspmDm == "302170100")) {
			zsxmDm = "10103";
		} else if ((zsxmDm == "30126" || zsxmDm == "30217")
				&& (zspmDm == "301260200" || zspmDm == "302170200")) {
			// 广告业文化事业建设费，取增值税的起征点
			zsxmDm = "10101";
		}
	}
...
function saveBeforeCheck() {
	...
	var rowDatas = grid.getAllNoDeleteGridData().trs;
	for (var i = 0; i < rowDatas.length; i++) {
		var sfysb = rowDatas[i].getValue("sfysb");
		if ("Y" == gt3.cfsbjybz && "Y" == sfysb) {
			continue;
		}
		...
		if (bqjmsfe > 0 && (ssjmxzDm == undefined || !$chk(ssjmxzDm))) {
			msgError += "税款所属期起" + sfkssqq + "至" + sfkssqz + ",征收品目为" + zspmMc
					+ "的减免税额大于0，但是没有对应的税收减免性质，请先选择税收减免性质" + "\n";
			if (msgError != "") {
		...
		for (var j = 0; j < rowDatas.length; j++) {
			...
			if (rowNum1 != rowNum) {
				var sfysb1 = rowDatas[j].getValue("sfysb");
				if ("Y" == gt3.cfsbjybz && "Y" == sfysb1) {
					continue;
				}
		...
		if (bqjmsfe / 1 <= 0 && $chk(ssjmxzDm)) {
			...
			cfsbMsg += "征收项目：" + zsxmMc + "<br/>征收品目：" + zspmMc + "<br/>";
			if ($chk(zszmDm)) {
				cfsbMsg += "征收子目：" + zszmMc + "<br/>";
			}
			cfsbMsg += "税款所属期起：" + sfkssqq + "<br/>税款所属期止：" + sfkssqz
		...
		if (gt3.fjsxmsbbz == 'Y') {
			if (fjsXmsbjy(zspmDm, sybh1)) {
				var cfsbMsg = "";
				cfsbMsg += "请修改序号：" + rowNum + "，征收项目：" + zsxmMc + "，征收品目："
						+ zspmMc + "，税款所属期起" + sfkssqq + "至" + sfkssqz
						+ "的记录<br/>";
...
function changeSsjmxzDm(option, selItem, obj) {
	...
	if ((bqynsfe / 1).round(2) != (jsfyj.multiple(fdsl)).round(2)) {
		jmse = getjmsebySsjmxzDm(selItem, jsfyj, (bqynsfe / 1)
				.subtract(bqyjsfe / 1), fdsl);
	} else {
		jmse = getjmsebySsjmxzDm(selItem, jsfyj, (jsfyj.multiple(fdsl))
				.subtract(bqyjsfe / 1), fdsl);
	}
	jmse = jmse.round(2);
...
function fjsXmsbjy(zspmDm, sybh1) {
	if (isFjszspm(zspmDm)
			&& (!$chk(sybh1) || sybh1 == null || sybh1 == undefined || sybh1 == "")) {
		return true;
	}
	return false;
}
...
//网报特色改造，内蒙地税小微企业地方水利建设基金减免提示
function xwqyJmts(zspmDm, jmxzDm, jsyj, ybtse, skssqq, skssqz,zsxmDm){
	if(("30216" == zsxmDm||"30203" == zsxmDm||"302210400" == zspmDm) && (jmxzDm == null || jmxzDm == "") && "Y" == xwqyjmtskg && ybtse > 0){
		queryDjhglxx();
		var yfkd = getMonths(skssqq, skssqz);
		var ysr = accDiv(jsyj, yfkd);
		if("Y" == xwqybz){
			swordAlert("您属于小型企业/微型企业，享受水利建设基金减免，请在“减免性质代码”中选择“0099129999︱小微企业免征地方教育附加︱其他”");
		}
	}
}
...
//网报特色改造，内蒙地税小微企业地方水利建设基金减免提示
function queryDjhglxx(skssqq, skssqz){
	//查询纳税人登记户归类
