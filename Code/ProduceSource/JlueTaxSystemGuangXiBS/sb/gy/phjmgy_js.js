...
/**
 * 专门给附加税用的方法，为了保持与核心一致
 * @param hasPhjmzg 资格
 * @return 普惠资格信息
 */
function checkPhZg1(hasPhjmzg,skssqq,skssqz){
	var xtgn_ctrl="SB175FxmtysbCtrl";
	var xtgn_ctrl1="SB402CztdsyssbCtrl,SB404FcssbCtrl"
	if(hasPhjmzg == "N"){//没有减免资格
		swordAlert("温馨提示：本属期该纳税人是增值税一般纳税人，不应享受该减免。");
		return "1";
	}else if(hasPhjmzg == "Y"){//有减免资格
		...
		if(gt3.ctrl=="SB025YhssbCtrl" || xtgn_ctrl1.indexOf(gt3.ctrl)!=-1){
			return "3";
		}
		if(qzDateCheck(skssqq, skssqz, '', '08')=='ok'){//除开房产税和城镇土地使用税，如果
			swordAlert("该纳税人申报属期内同时存在一般人和小规模两种纳税人状态，若使用该申报功能，系统会全额计算减征额，请自行折算并修改减征额。");
