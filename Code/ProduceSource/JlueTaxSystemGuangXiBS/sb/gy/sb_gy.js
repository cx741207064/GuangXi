...
function getMrjmseBySsjmxz(jmzlxDm,jsyj,ynse,fdsl,jmfd,jmed,jmsl)
{
	...
	///	flag = true;
	//}
	if(GyFormatRound(ynse,2) != GyFormatRound(jsyj.multiple(fdsl),2)){//非按照税率计税的情况,此时对于税率减免和幅度减免不做减免税额计算，需要前台手工录入减免税额
...
function toDsfjssb(gdsyybz,xtgnDm,skssqq,skssqz,nsrxxVO){
	var mesg = "";
	...
	if($chk(mesg)&&mesg.length>0){
		swordConfirm(mesg, {
			okBtnName : "是",
			cancelBtnName : "否",
			onOk : function() {
			dsfjssb(gdsyybz,xtgnDm,skssqq,skssqz,nsrxxVO);
		},
		onCancel : function() {
			return;
		},
...
function dsfjssb(gdsyybz,xtgnDm,skssqq,skssqz,nsrxxVO){
	var nsrmc = nsrxxVO.nsrmc;
	var ssdabh = nsrxxVO.ssdabh;
	var kzztdjlxDm = nsrxxVO.kzztdjlxDm;
	var kqccsztdjbz = nsrxxVO.kqccsztdjbz;
	var nsrData = "nsrmc="+nsrmc+"&ssdabh="+ssdabh+"&kzztdjlxDm="+kzztdjlxDm+"&kqccsztdjbz="+kqccsztdjbz+"&skssqq="+skssqq+"&skssqz="+skssqz;
