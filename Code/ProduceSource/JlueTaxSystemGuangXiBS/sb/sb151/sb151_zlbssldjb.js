...
var zlbsxxcjnb_nm = "N";// 内蒙年报需求提示信息控制开关
...
function init(resData) {
	zlbsdl = $w("nsrxxMap").getValue('zlbsdlDm').code;
	// 调用此方法隐藏“申报错误更正”按钮
	//hidecwgzbtn();
	...
	zlbsxxcjnb_nm = resData.getAttr("zlbsxxcjnb_nm");
	...
	if (zlbsdl == "ZL1005" || zlbsdl == "ZL1004") {
		document.getElementById("dcnd").style.display = "";
		$w("sbToolBar").setHide("zhcwbbbs");
	}
...
function queryNsrxxSuccess(nsrParam, zrrParam) {
	fddbrxm = nsrParam.fddbrxm.value;
...
function getKjzdba(q, z, callback) {
	...
	queryBtn.pushData("zlbsdlDm", zlbsdl);
...
function initSsq(req, resData) {
	...
	kjzdzzDm = pc.getResData("kjzdzzDm", resData).value;
		...
		if (needCwzdba()) {
			...
			if ($chk(zlbsxxcjnb_ln) && zlbsxxcjnb_ln == "Y"
				...
				if (nsrcwzdxxList.length == 0) {
					if (zlbsxxcjnb_nm == "Y") {
						swordAlert('您没有财务会计制度备案信息,请及时办理备案，路径：【综合信息报告】--【制度信息报告】--【财务会计制度备案】');
					} else {
						swordAlert('您没有财务会计制度备案信息,请及时联系主管税务机关进行备案!');
					}
					...
					for ( var i = 0; i < nsrcwzdxxList.length; i++) {
						...
						var baleg = babj.substr(babj.length / 1 - 2,
								babj.length / 1);
						if (cwkjbblxDmTemp == "01" && baleg != "01") {// 判断纳税人是否同时存在三种备案信息
							babj = babj + cwkjbblxDmTemp;
						}
						if (cwkjbblxDmTemp == "02" && baleg != "02") {// 判断纳税人是否同时存在三种备案信息
							babj = babj + cwkjbblxDmTemp;
						}
						if (cwkjbblxDmTemp == "03" && baleg != "03") {// 判断纳税人是否同时存在三种备案信息
							babj = babj + cwkjbblxDmTemp;
						}
					}
					if (babj.length == 6) {// 如果存在三种备案数据则提示纳税人修改
						swordAlert('您同时存在月、季、年的财务会计制度备案信息，备案信息错误,请联系主管税务机关进行修改!');
						return;
					}
					if (bz == 0) {
						if (zlbsxxcjnb_nm == "Y") {
							swordAlert('您没有季度或月度财务会计报表备案信息,请及时办理备案，路径：【综合信息报告】--【制度信息报告】--【财务会计制度备案】');
						} else {
							swordAlert('您没有季度或月度财务会计制度备案信息,请及时联系主管税务机关进行备案!');
						}
						$w('sbToolBar').setDisabled('next');
						$w("nsrxxMap").disable();
						return;
			...
			if (nsrcwzdxxList.length == 0) {
				...
				if ($chk(kjzdabz_hb) && kjzdabz_hb == "Y"
					...
					swordAlert("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;您未进行财务会计制度备案，如需填写财务报表请通过电子税务局在线办税功能申请或至税务机关办税服务厅按照公告内容进行财务会计制度备案。<br>"
							+ gkdz);
					$w('sbToolBar').setDisabled('next');
					$w("nsrxxMap").disable();
					return;
				...
				if ($chk(kjzdbz) && kjzdbz == "Y" && kjzdbz != undefined) {
					swordConfirm(
							"您没有财务会计制度备案信息,请先进行财务会计制度备案才能进行纳税申报",
							{
								okBtnName : "是",
								cancelBtnName : "否",
								onOk : function() {
	...
	if ($chk(kjzdabz_hb) && kjzdabz_hb == "Y" && kjzdabz_hb != undefined) {
		if (kjzdzzDm != '101' && kjzdzzDm != '102' && kjzdzzDm != '201'
			...
			if ($chk(bzgkba_hb) && bzgkba_hb == "Y" && bzgkba_hb != undefined) {
				swordAlert("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;您的财务制度备案与国地税联合公告确定的财务制度备案报表范围不符，请通过电子税务局在线办税功能申请或至税务机关办税服务厅按照公告内容修改财务会计制度备案信息。<br>"
						+ gkdz);
				$w('sbToolBar').setDisabled('next');
				$w("nsrxxMap").disable();
				return;
...
function changessqz(xlChangeBz) {
	...
	if (!((parseInt(qDates[1], 10) == 1 && parseInt(zDates[1], 10) == 3)
		...
		swordAlert("所属期不合法，必须为月、季或年！");
		$w("nsrxxMap").setValue("ssqz", "");
		return;
	}
	// 网报特色 济南需求 通过开关控制112
	if (fbaxlkg == "Y" && $chk(djxh) && "N" == _gIsQhjt && "ZL1001" == zlbsdlDm) {
		$w("nsrxxMap").setValue("zlbsxlDm", zlbsxlDm.code); // 开关打开先清空一下页面小类避免出现未采集过小类显示出来的情况
		getKjzdba(ssqq, ssqz, "initSsq");
	}
	if (fbaxlkg != "Y" && $chk(djxh) && "N" == _gIsQhjt && "ZL1001" == zlbsdlDm) {
		getKjzdba(ssqq, ssqz, "initSsq");
	...
	if (needCwzdba()) {
		// 当选择年报的时候，校验有效期年度是否包含所属期年度，如果包含报送期限直接返回10
		if ($chk(yxqq) && "" != yxqq && $chk(yxqz) && "" != yxqz
				&& splitStr(yxqq)[0] <= splitStr(ssqq)[0]
				&& splitStr(ssqz)[0] <= splitStr(yxqz)[0]
				&& splitStr(ssqq)[1] == "01" && splitStr(ssqz)[1] == "12"
				&& $chk(zlbsxxcjnb_ln) && zlbsxxcjnb_ln == "Y"
				&& zlbsxxcjnb_ln != undefined) {
...
function getNsqxDmFromCwkjbblx(cwkjbblx) {
	...
	if (1 == cwkjbblxInt) { // 月报表
		...
		if ($chk(kjzdabz_hb) && kjzdabz_hb == "Y" && kjzdabz_hb != undefined) {
			if (bsqxDm == "06") {
				swordAlert("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;您的财务报表所属期类型是月报，与公告确定的季报和年报所属期类型不符，请通过电子税务局在线办税功能申请或至税务机关办税服务厅按照公告内容修改财务会计制度备案信息。<br>"
						+ gkdz);
				$w('sbToolBar').setDisabled('next');
				$w("nsrxxMap").disable();
				return;
...
function checkSsqz(xlChangeBz) {
	...
	if ($chk(zlbsxxcjnb_ln) && zlbsxxcjnb_ln == "Y"
		...
		if (nsrcwzdxxList.length == 0) {
			if (zlbsxxcjnb_nm == "Y") {// 内蒙需求，修改提示内容
				swordAlert('您没有财务会计制度备案信息,请及时办理备案，路径：【综合信息报告】--【制度信息报告】--【财务会计制度备案】');
			} else {
				swordAlert('您没有财务会计制度备案信息,请及时联系主管税务机关进行备案!');
			}
	...
	if (nsrcwzdxxList.length == 0) {
		...
		// 都配置为N时，修改财务报表所属起时总提示“您的财务会计制度备案为请填写正确的所属期起止！”
		// 调整所属起js方法，当参数 Z0000096Z02000050 山西版 Z0000096Z02000077
		// 全部配置N时，提示于金三系统操作相同的“您在此期间没有财务会计制度备案信息!”
		if ($chk(fbaxlkg) && fbaxlkg == "Y" && fbaxlkg != undefined) { // 济南地税需求如果会计制度备案过期或无备案时走参数配置表。无需提示“您在此期间没有财务会计制度备案信息”。
			// 2018-01-29不进行您在此期间没有财务会计制度备案信息提示.
		} else {
			swordAlert('您在此期间没有财务会计制度备案信息!');
			return;
		}
	...
	if (zlbsxxcjnb_ln != "Y") {
		if ($chk(fbaxlkg) && fbaxlkg == "Y" && fbaxlkg != undefined
				&& bblxStr == "") {
			// 2018-01-29济南地税 如果 bblxstr 为空时不考虑 不提示您的财务会计制度备案为" + bblxStr +
			// "请填写正确的所属期起止！。
		} else if ($chk(fbaxlkg) && fbaxlkg == "Y" && fbaxlkg != undefined
				&& bblxStr != "") {
			// 2018-01-29所属期止校验没通过提示 济南地税 bblxStr 不为空提示
			$w("nsrxxMap").setValue("ssqz", "");
			swordAlert("您的财务会计制度备案为" + bblxStr + "请填写正确的所属期起止！");
			return false;
		} else {
			// 所属期止校验没通过提示
			$w("nsrxxMap").setValue("ssqz", "");
...
function querySuccess(req, resData) {
	...
	fddbrxm = $w("nsrxxMap").getValue('fddbrxm');
	var ssqq = $w("nsrxxMap").getValue('ssqq');
	var ssqz = $w("nsrxxMap").getValue('ssqz');
	if (reCode1 == "1") {
		var msg = "";
 ...
 */
function reset() {
	$w('nsrxxMap').reset(['ssqq','ssqz','dcnd','nsrsbh','nsrmc','zlbsdlDm']);
	$w('sbToolBar').setEnabled('next');
	$w('nsrxxMap').enable('ssqz');
	$w('nsrxxMap').enable('zlbsxlDm');
	$w('nsrxxMap').enable('ssqq');
