...
function setHx(a){
	...
	if(a=="01"){
		...
		if(sfcfcj=='Y'||sbdr=='sbdr'){
			$('table01iframe').contentWindow.setCfcjInitValue(nsrxxForm2cj,cjhxfblrbxx,lrbCjcgbj,nsrsbh,nsrmc,bsrq,skssqq,skssqz,bbssq);
		}else{
			//nsrsbh,nsrmc,bsrq,skssqq,skssqz,sqlrblist,bbssq
			$('table01iframe').contentWindow.setInitValueYbOpen(nsrxxFormData);
		}
	}else if(a=="02"){
		// 现金流量表
		if(sfcfcj=='Y'||sbdr=='sbdr'){
			$('table02iframe').contentWindow.setCfcjInitValue(nsrxxForm3cj,cjhxfbxjllbxx,xjllbCjcgbj,nsrsbh,nsrmc,bsrq,skssqq,skssqz);
		}else{
			$('table02iframe').contentWindow.setInitValue(nsrsbh,nsrmc,bsrq,skssqq,skssqz,xjllblist);
		}
	}else if(a=="03"){// 利润表年报表
		if((sfcfcj=='Y'||sbdr=='sbdr')&&(nsrxxForm4cj!=null||nsrxxForm4cj!=undefined)){
			$('table03iframe').contentWindow.setCfcjInitValue(nsrxxForm4cj,cjhxfblrbxxNbb,sfcfcj,nsrsbh,nsrmc,bsrq,sqsqqnb,skssqznb,bbssq,nsrxxFormData);
		}else{
			$('table03iframe').contentWindow.setInitValueNbOpen(nsrsbh,nsrmc,bsrq,skssqq,skssqz,bbssq);
		}
	}else if(a=="04"){
		// 现金流量表年报
		if((sfcfcj=='Y'||sbdr=='sbdr')&&(nsrxxForm5cj!=null||nsrxxForm5cj!=undefined)){
			$('table04iframe').contentWindow.setCfcjInitValue(nsrxxForm5cj,cjhxfbxjllbxxNbb,sfcfcj,nsrsbh,nsrmc,bsrq,sqsqqnb,skssqznb);
		}else{
			$('table04iframe').contentWindow.setInitValue(nsrsbh,nsrmc,bsrq,skssqq,skssqz);
		}
	}else if(a=="05"){//存货的披露
		if(sfcfcj=='Y'||sbdr=='sbdr'){
			$('table05iframe').contentWindow.setCfcjInitValue(cjhxfbChdpl,nsrsbh,nsrmc,bsrq,skssqq,skssqz);
		}else{
			$('table05iframe').contentWindow.setInitValue(nsrsbh,nsrmc,bsrq,skssqq,skssqz);
		}
	}else if(a=="06"){//短期投资的披露
		if(sfcfcj=='Y'||sbdr=='sbdr'){
			$('table06iframe').contentWindow.setCfcjInitValue(cjhxfbDqtzdpl,nsrsbh,nsrmc,bsrq,skssqq,skssqz);
		}else{
			$('table06iframe').contentWindow.setInitValue(nsrsbh,nsrmc,bsrq,skssqq,skssqz);
		}
	}else if(a=="07"){//固定资产的披露
		if(sfcfcj=='Y'||sbdr=='sbdr'){
			$('table07iframe').contentWindow.setCfcjInitValue(cjhxfbGdzcdpl,nsrsbh,nsrmc,bsrq,skssqq,skssqz);
		}else{
			$('table07iframe').contentWindow.setInitValue(nsrsbh,nsrmc,bsrq,skssqq,skssqz);
		}
	}else if(a=="08"){//利润分配表
		if(sfcfcj=='Y'||sbdr=='sbdr'){
			$('table08iframe').contentWindow.setCfcjInitValue(snlrfpbList, cjhxfbLrfpb,nsrsbh,nsrmc,bsrq,skssqq,skssqz);
		}else{
			$('table08iframe').contentWindow.setInitValue(snlrfpbList, nsrsbh,nsrmc,bsrq,skssqq,skssqz);
		}
	}else if(a=="09"){//应付职工薪酬明细表
		if(sfcfcj=='Y'||sbdr=='sbdr'){
			$('table09iframe').contentWindow.setCfcjInitValue(cjhxfbYfzgxcmxb,nsrsbh,nsrmc,bsrq,skssqq,skssqz);
		}else{
			$('table09iframe').contentWindow.setInitValue(nsrsbh,nsrmc,bsrq,skssqq,skssqz);
		}
	}else if(a=="10"){//应交税费明细表
		if(sfcfcj=='Y'||sbdr=='sbdr'){
			$('table10iframe').contentWindow.setCfcjInitValue(cjhxfbYjsfmxb,nsrsbh,nsrmc,bsrq,skssqq,skssqz);
		}else{
			$('table10iframe').contentWindow.setInitValue(nsrsbh,nsrmc,bsrq,skssqq,skssqz);
		}
	}
}
/**
